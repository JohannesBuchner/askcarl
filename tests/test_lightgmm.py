...
def test_weights():
    ...
    assert_allclose(gmm.means_, gmm2.means_)
    assert_allclose(gmm.precisions_cholesky_, gmm2.precisions_cholesky_)
    assert_allclose(gmm.covariances_, gmm2.covariances_)
    ...
    assert_allclose(gmm3.means_, gmm4.means_)
    assert_allclose(gmm3.precisions_cholesky_, gmm4.precisions_cholesky_)
    assert_allclose(gmm3.covariances_, gmm4.covariances_)
    ...
    assert_allclose(gmm5.means_, gmm4.means_)
    assert_allclose(gmm5.precisions_cholesky_, gmm4.precisions_cholesky_)
    assert_allclose(gmm5.covariances_, gmm4.covariances_)
...
def test_single_gauss():
    ...
    for D in [2, 5, 20]:
        ...
        assert (labels == 0).all()
        assert_allclose(Y.mean(axis=0), 0, atol=0.04)
        assert_allclose(Y.std(axis=0), 1, atol=0.04)
        
        gmm_ref = GaussianMixture(1)
        gmm_ref.fit(X)
        score_ref = gmm_ref.score(X)
        assert_allclose(score, score_ref)
...
def test_two_gauss():
    ...
    for D in [2, 5, 20]:
        ...
        print(A.mean(axis=0))
        print(B.mean(axis=0))
        assert_allclose(A.mean(axis=0), 0, atol=0.02 * D)
        assert_allclose(B.mean(axis=0), 10, atol=0.02 * D)
        assert_allclose(A.std(axis=0), 1, atol=0.02 * D)
        assert_allclose(B.std(axis=0), 1, atol=0.02 * D)
        gmmsk = gmm.to_sklearn()
        for attribute in 'means_', 'precisions_cholesky_', 'weights_', 'covariances_':
            assert_allclose(getattr(gmmsk, attribute), getattr(gmm, attribute))
        ...
        assert_allclose(mix.weights, gmm.weights_)
        assert_allclose(mix.log_weights, np.log(gmm.weights_))
        for i, comp in enumerate(mix.components):
            assert_allclose(comp.mean, gmm.means_[i])
            assert_allclose(comp.cov, gmm.covariances_[i])
